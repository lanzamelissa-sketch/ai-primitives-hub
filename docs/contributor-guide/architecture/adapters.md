...
>
> **MCP extraction (GitHub):** `processSingleRelease` reads MCP data from the deployment manifest using two keys: `mcp.items` (new format, written by `generate-manifest.js`) and `mcpServers` (legacy fallback). When `mcp.inputs` is present it is attached to the bundle as `mcpInputs`, enabling VS Code to prompt the user for secrets and configurable values at install time. This mirrors the behaviour of `AwesomeCopilotAdapter`, which reads the same keys from `.collection.yml` files directly.
>
